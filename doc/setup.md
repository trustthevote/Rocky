...
In addition to states.yml, the state-import process reads the data/zip_codes/county_addresses.csv file to create a list of zip-code to mailing-address mappings for registrars. The county_addresses.csv file is created using the API from the US Vote Foundation (http://eod.usvotefoundation.org). The CSV file can be generated by running 
