...
set :rake, 'sudo bundle exec rake'
...
namespace :deploy do
  ...
  task :import_states_csv, :roles => [:app] do
    run <<-CMD
      cd #{latest_release} &&
      sudo bundle exec rake import:states CSV_FILE=db/bootstrap/import/states.csv
    CMD
  end
  ...
  task :symlink_branding, :roles => [:app, :util], :except => {:no_release => true} do
    run "cd #{latest_release} && sudo bundle exec rake branding:symlink"
  end
...
namespace :import do
  ...
  task :states, :roles => :app do
    ...
    top.upload local_path, remote_path, :via => :scp
    run "cd #{current_path} && sudo bundle exec rake import:states CSV_FILE=#{remote_path}"
